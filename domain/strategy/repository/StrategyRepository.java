...
public interface StrategyRepository extends JpaRepository<Strategy, Long> {
    ...
    // Summary queries - Updated for new field structure
    ...
    @Query("SELECT s FROM Strategy s WHERE " +
            ...
            "(:primaryName IS NULL OR LOWER(s.primaryName) LIKE LOWER(CONCAT('%', :primaryName, '%'))) AND " +
            "(:secondaryName IS NULL OR LOWER(s.secondaryName) LIKE LOWER(CONCAT('%', :secondaryName, '%'))) " +
            "ORDER BY s.createdAt DESC")
    Page<Strategy> findByMultiLanguageNames(@Param("primaryName") String primaryName,
